...
```bash
git clone https://github.com/Catkin-714/AegisX.git AegisX
cd AegisX
```
...
Compose 会构建 `aegisx-crypten:local` 和默认 MCU 镜像。完整 BERT 的 MCU session 使用单独标签 `aegisx-mcu:bert-session-fixed`。
