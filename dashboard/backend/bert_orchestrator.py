...
def _attention_from_text(text: str) -> list[dict[str, Any]]:
    ...
    for i, w in enumerate(words):
        wt = ((seed >> (i * 3)) % 100) / 100
        weights.append(wt)
    total = sum(weights) if sum(weights) > 0 else len(weights)
    return [{"word": word, "weight": round(wt / total, 4)} for word, wt in zip(words[:32], weights[:32])]
...
def infer_mcu_docker(text: str, max_seq_len: int) -> dict[str, Any]:
    ...
    cmd = [
        ...
        "--image",
        os.environ.get("MCU_DASHBOARD_IMAGE", "aegisx-mcu:bert-session-fixed"),
        "--batch",
        "1",
        "--seq",
        str(max_seq_len),
...
def compare(text: str, launch: str, max_seq_len: int, modes: list[str] | None = None) -> dict[str, Any]:
    ...
    from transformer.mcu_nonlinear import HAS_MCU_RUST
    if "mcu_rust" in selected_modes and not HAS_MCU_RUST and launch == "process":
        selected_modes = [m for m in selected_modes if m != "mcu_rust"]
    results = []
