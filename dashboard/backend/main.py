...
def _attention_from_text(text: str):
    ...
    for i, w in enumerate(words):
        h = int(hashlib.md5(f"{text}|{i}".encode()).hexdigest()[:8], 16)
        wt = (h % 80 + 10) / 100.0
        weights.append(wt)
    total = sum(weights)
    return [{"word": w, "weight": round(wt / total, 3)} for w, wt in zip(words[:48], weights[:48])]
...
def run_scenario_compare(req: ScenarioCompareRequest):
    try:
        if req.scenario == "sentiment":
            from bert_orchestrator import compare, _normalize
            import re, time
            has_chinese = bool(re.search(r'[\u4e00-\u9fff]', req.text))
            if has_chinese:
                from infer_engine import classify_by_rule
                label, dist = classify_by_rule(req.text, "sentiment")
                probs = [d["prob"] / 100.0 for d in dist]
                t = time.strftime("%H:%M:%S")
                logs = [
                    {"timestamp": t, "sender": "HOST", "receiver": "HOST", "type": "规则", "detail": "中文关键词规则（bert-base-uncased 不支持中文，使用规则兜底）", "bytes": len(req.text.encode("utf-8"))},
                ]
                p = _normalize(text=req.text, mode="plaintext", launch=req.launch, method="中文关键词规则", probabilities=probs, elapsed_seconds=0.0, logs=logs)
                c = _normalize(text=req.text, mode="crypten", launch=req.launch, method="中文关键词规则（填充路径）", probabilities=probs, elapsed_seconds=0.0, logs=logs)
                m = _normalize(text=req.text, mode="mcu_rust", launch=req.launch, method="中文关键词规则（填充路径）", probabilities=probs, elapsed_seconds=0.0, logs=logs)
                results = [p, c, m]
                all_logs = []
                for item in results:
                    all_logs.extend(item["logs"])
                return {
                    "success": True, "mode": "scenario_compare",
                    "scenario": "情感分析", "launch": req.launch,
                    "input": req.text, "results": results, "logs": all_logs,
                    "elapsed_seconds": 0.0, "success_count": 3, "total_count": 3,
                    "attention": _attention_from_text(req.text),
                }
...
def rust_verify():
    ...
    if hp_online and HAS_MCU_RUST:
        prg_ok = verify_mcu_rust_prg(100)
        return {"available": True, "prg_ok": prg_ok, "message": "Rust HP 运行中，PRG 对齐通过" if prg_ok else "Rust HP 运行中，PRG 校验失败"}
    if hp_online:
        return {"available": True, "prg_ok": False, "message": "Rust HP 运行中（mcu_rust Python binding 未编译）"}
    return {"available": False, "prg_ok": False, "message": "Rust HP 离线"}
