...
docker build -f docker/Dockerfile.mcu.bert_session -t aegisx-mcu:bert-session-fixed .
```

The compose build creates `aegisx-crypten:local` and the default MCU image. The full BERT MCU session uses the `aegisx-mcu:bert-session-fixed` tag.
...
```bash
export AEGISX_BERT_BASE_HOST=/abs/path/bert-base-uncased
export AEGISX_CHECKPOINTS_HOST=/abs/path/checkpoints
```

PowerShell:

```powershell
$env:AEGISX_BERT_BASE_HOST = "D:\models\bert-base-uncased"
$env:AEGISX_CHECKPOINTS_HOST = "D:\models\checkpoints"
```
...
python experiments/docker_bert_full/run_mcu_bert_session_docker.py \
  --image aegisx-mcu:bert-session-fixed \
  --batch 1 \
  --seq 16 \
  --hidden 768 \
  --heads 12 \
