...
- Edge validation on 2026-06-30:
  ...
  - Retest passed after rebuilding `aegisx-mcu:bert-session-fixed`: API sentiment service compare returned `3/3` in `45.0s`; Edge `index_new.html` sentiment service compare returned `3/3` in `38.6s`; default `index.html` MCU service inference returned in `15.7s`. No browser network errors and no stuck `三路比较中` state.
