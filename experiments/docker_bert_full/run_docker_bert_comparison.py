...
def docker_bind_root(out_host: Path, timestamp: str) -> Path:
    override = os.environ.get("AEGISX_DOCKER_BIND_HOST")
    if override:
        path = Path(override)
    elif os.name == "nt":
        path = Path(tempfile.gettempdir()) / "aegisx_docker_bert_out" / timestamp
    else:
        path = out_host
...
def docker_model_roots(stage_models: bool) -> tuple[Path | None, Path | None]:
    override_base = os.environ.get("AEGISX_BERT_BASE_HOST")
    override_ckpt = os.environ.get("AEGISX_CHECKPOINTS_HOST")
    ...
    cache_root = Path(os.environ.get("AEGISX_DOCKER_MODEL_CACHE", Path(tempfile.gettempdir()) / "aegisx_docker_bert_models"))
...
def run_crypten_docker(
    ...
    if bert_base_host and checkpoints_host:
        cenv["AEGISX_BERT_BASE_HOST"] = str(bert_base_host)
        cenv["AEGISX_CHECKPOINTS_HOST"] = str(checkpoints_host)
