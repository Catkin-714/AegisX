...
def docker_bind_root(out: Path, timestamp: str) -> Path:
    ...
    if override:
        ...
        path = Path(tempfile.gettempdir()) / "aegisx_mcu_bert_session" / timestamp
    else:
        path = out / "shared"
...
def docker_share_root(out: Path, timestamp: str) -> Path:
    override = os.environ.get("AEGISX_DOCKER_BERT_SHARES_HOST")
    if override:
        path = Path(override)
    elif os.name == "nt":
        path = Path(tempfile.gettempdir()) / "aegisx_mcu_bert_shares" / timestamp
    else:
...
def main() -> int:
    ...
    parser.add_argument("--image", default="aegisx-mcu:bert-session")
    ...
    if share_bind is not None:
        env["AEGISX_BERT_SHARES_HOST"] = str(share_bind)
