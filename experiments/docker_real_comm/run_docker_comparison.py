...
def docker_bind_root(out_host: Path, timestamp: str) -> Path:
    ...
    if override:
        ...
        path = Path(tempfile.gettempdir()) / "aegisx_docker_out" / timestamp
    else:
        path = out_host
...
def main() -> int:
    ...
    base_env["AEGISX_OUT_HOST"] = str(bind_host)
    base_env["MCU_CUDA_MIN_OPS"] = str(args.mcu_cuda_min_ops)
    if args.mcu_gpu:
        base_env["MCU_DOCKERFILE"] = "docker/Dockerfile.mcu.cuda"
        base_env["MCU_IMAGE"] = "aegisx-mcu:cuda"

    if not args.skip_build:
        print("[docker-compare] building images")
        run([*compose_base("aegisx_build", args.mcu_gpu), "build", "mcu-hp", "crypten-r0"], base_env, timeout=1800)
